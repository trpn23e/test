...
class DTI extends Component {
  ...
  _updtDtiInterfaceMsgStatusN = (ev) => {
    ev.nativeEvent.stopImmediatePropagation();
    updtDtiInterfaceMsgStatusN({testParam: this.state.inputValue}).then(response => {
      // let data = response.data
      alert('DTI Interface Table Updated')
    }).catch(err => {
      alert(err)
    })
  }
          ...
          */}
          ...
          <Card className="box-card" style={{marginBottom:20}}>
            ...
            <div style={styles.divDefault}>
              ...
              <Button type="primary" onClick={this._goWebpackAnalyzer}>WebPack Analyzer</Button>
              <br></br><br></br>
            ...
            <div style={styles.divDefault}>
              <Fragment>XXSB_DTI_INTERFACE 메시지 상태를 N으로 업데이트 : </Fragment>
              <Button type="primary" onClick={this._updtDtiInterfaceMsgStatusN}>업데이트</Button><br></br>
              <Fragment>JMS 호출 테스트(DTI Ountbound(Adaptor - Router - Connector) -> SB) : </Fragment>
              <Button type="primary" onClick={this._getListToTestDtiJMS}>호출</Button><br></br>
              <Fragment>JMS 호출 테스트(SB결과 -> Inbound Router) : </Fragment>
